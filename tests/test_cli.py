...
def test_cli_init_catalog_task_and_doctor(tmp_path) -> None:
    ...
    init_result = runner.invoke(app, ["init", str(vault_path), "--category-template", "minimal"])
...
def test_cli_manual_category_and_tag_commands_do_not_mutate_revisions(tmp_path) -> None:
    ...
    tag_add_result = runner.invoke(
        app, ["tag", "add", "important", "--type", "topic", "--vault", str(vault_path)]
    )
...
def test_cli_doc_list_filters_status_category_and_tag(tmp_path) -> None:
    ...
    tag_create = runner.invoke(
        app, ["tag", "add", "m5-visible", "--type", "topic", "--vault", str(vault_path)]
    )
